/**
 * OTWheel - OpenToonz-style Color Wheel Library
 * A reusable HSV color picker with hexagonal hue wheel and SV triangle
 * 
 * @author Based on OpenToonz color picker design
 * @version 1.0.0
 */
...
(function(global) {
  ...
  /* ========================================
   * OTColor Class - Color representation with getters/setters
   * ======================================== */

  /**
   * OTColor - Represents a color with HSV and RGB access
   */
  class OTColor {
    constructor() {
      this._h = 0;   // 0-360
     ...
     * @param {Function} callback - Called when color changes, receives this OTColor
    ...
    _notify() {
      for (const listener of this._listeners) {
        try {
          ...
          console.error('OTColor listener error:', e);
        }
      }
    }
    ...
    /**
     * Clone this color
     * @returns {OTColor} New OTColor instance with same values
     */
    clone() {
      const c = new OTColor();
      c._h = this._h;
    ...
    /**
     * Copy values from another OTColor
     * @param {OTColor} other - Color to copy from
     */
    copyFrom(other) {
      this._h = other._h;
      this._s = other._s;
    ...
    toString() {
      return `OTColor(h=${this._h.toFixed(1)}, s=${this._s.toFixed(2)}, v=${this._v.toFixed(2)})`;
    }
  }

  /* ========================================
   * OTWheel Class - The Color Wheel Widget
   * ======================================== */

  /**
   * OTWheel - OpenToonz-style HSV Color Wheel
   */
  class OTWheel {
    /**
     * Create a new OTWheel
     ...
     * @param {OTColor} [options.color] - Initial color (creates new if not provided)
     */
    constructor(options = {}) {
      // Get container
      ...
      if (!this._container) {
        throw new Error('OTWheel: container element required');
      }
      ...
      // Color state
      this._color = options.color || new OTColor();
    ...
    /** @returns {OTColor} The current color */
    get color() { return this._color; }

    set color(value) {
      if (value instanceof OTColor) {
        this._color = value;
        this._color.onChange(() => this.render());
        this.render();
      } else {
        throw new Error('OTWheel: color must be an OTColor instance');
      }
    }
  ...
  /* ========================================
   * OTSlider Class - Color Slider Widget
   * ======================================== */

  /**
   * OTSlider - OpenToonz-style color slider
   */
  class OTSlider {
    /**
     * Create a new OTSlider
     * @param {Object} options - Configuration options
     * @param {HTMLElement|string} options.container - Container element or selector
     * @param {string} options.channel - Channel: 'h', 's', 'v', 'a', 'r', 'g', 'b'
     * @param {OTColor} options.color - Color to bind to
    ...
    constructor(options = {}) {
      ...
      if (!this._container) {
        throw new Error('OTSlider: container element required');
      }

      if (!options.color) {
        throw new Error('OTSlider: color (OTColor instance) required');
      }
      ...
      if (!this._channelConfig[this._channel]) {
        throw new Error(`OTSlider: invalid channel "${this._channel}"`);
      }

      this._config = this._channelConfig[this._channel];

      // Alpha value (stored separately since OTColor doesn't have alpha)
      this._alpha = 255;
    ...
    /** @returns {OTColor} The bound color */
    get color() { return this._color; }
    ...
    _buildUI() {
      ...
      this._wrapper.style.cssText = 'display:flex;align-items:flex-start;font-family:sans-serif;font-size:14px;margin-bottom:-3px;';
      ...
      if (this._showLabel) {
        ...
        this._label.style.cssText = 'width:16px;height:24px;line-height:24px;color:#ddd;font-weight:bold;text-align:center;margin-right:4px;';
      ...
      if (this._showInput) {
        ...
        this._input.style.cssText = 'width:40px;height:24px;padding:0 4px;background:#222;color:#fff;border:1px solid #555;border-radius:3px;text-align:center;font-family:monospace;font-size:12px;box-sizing:border-box;margin-right:4px;';
      ...
      if (this._showArrows) {
        ...
        this._leftArrow.style.cssText = 'width:20px;height:24px;padding:0;background:#333;color:#aaa;border:1px solid #555;cursor:pointer;font-size:8px;display:flex;align-items:center;justify-content:center;box-sizing:border-box;margin-right:4px;';
      ...
      this._barContainer.style.cssText = `position:relative;width:${this._width}px;height:${this._height + 8}px;`;

      // Canvas for gradient
      this._canvas = document.createElement('canvas');
      this._canvas.width = this._width;
      this._canvas.height = this._height;
      this._canvas.style.cssText = 'display:block;box-sizing:border-box;margin-top:2px;';
      this._ctx = this._canvas.getContext('2d');
      ...
      if (this._showArrows) {
        ...
        this._rightArrow.style.cssText = 'width:20px;height:24px;padding:0;background:#333;color:#aaa;border:1px solid #555;cursor:pointer;font-size:8px;display:flex;align-items:center;justify-content:center;box-sizing:border-box;margin-left:5px;';
    ...
    _updateFromMouse(e) {
      const rect = this._canvas.getBoundingClientRect();
      const x = Math.max(0, Math.min(this._width, e.clientX - rect.left));
      const ratio = x / this._width;
    ...
    _setValue(value) {
      switch (this._channel) {
        ...
        case 'a': this._alpha = value; this._update(); break;
  ...
  OTWheel.hsvToRgb = hsvToRgb;
  OTWheel.rgbToHsv = rgbToHsv;
  OTWheel.rgbToHex = rgbToHex;
  OTWheel.hexToRgb = hexToRgb;

  // Export classes
  OTWheel.Color = OTColor;
  OTWheel.Slider = OTSlider;

  // Export to global/module
  if (typeof module !== 'undefined' && module.exports) {
    module.exports = { OTWheel, OTColor, OTSlider };
  } else {
    global.OTWheel = OTWheel;
    global.OTColor = OTColor;
    global.OTSlider = OTSlider;
  }
